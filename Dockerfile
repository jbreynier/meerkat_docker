...
# Install Primer3
RUN cd /usr/local/ && \
    wget https://sourceforge.net/projects/primer3/files/primer3/2.2.0-alpha/primer3-2.2.0-alpha.tar.gz && \
...
RUN cd /usr/local/Meerkat/src/ && \
    ...
    make && \
    mv ./bamreader ../../bin/ && \
    export LD_LIBRARY_PATH=/usr/local/Meerkat/src/mybamtools/lib
